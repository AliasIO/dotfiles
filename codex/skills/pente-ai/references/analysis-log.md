...
## 2026-04-28 - Soft counter-threat must not hand over delayed capture-fork setup

- Game: `local-ai-aeb88e93-62ba-4517-8f3f-78ed6d331a1c`, first avoidable cause at computer move 8 after human `(9,8)#161`.
- Symptom: computer chose soft counter-threat `(8,7)#141` with no immediate human win on board. Human reply `(8,6)#122` created a capture setup on the newly formed pair and the later game collapsed into capture defense, instant-loss defense, then a two-win terminal position.
- Root cause: `safeCounterThreatMoves` treated a quiet counter-threat as safe when there was no immediate capture threat, but it did not score the opponent's next-move capture setup on the just-created pair. Delayed capture setups are especially bad when the setup reply also creates open-three fork defense pressure.
- Fix or decision: add a soft delayed-capture setup penalty for counter-threat shortcuts, using the played stone's open-pair setup replies and their resulting fork/open-four/open-three pressure. If all counter-threat moves are rejected for this reason, use a normal fallback excluding the rejected move. Also preserve the prior safe-forcing-four rule only when the opponent has no active capture setup, so capture-race defenses still beat speculative counter-wins.
- Regression probe: compact fixture `counter threat must avoid delayed capture setup` rejects `(8,7)#141`; focused probe now selects `(10,9)#181` or `(8,10)#198` depending on build timing, both with no immediate human win or capture threat. Serial compact bench passed 21/21 in 85.14s; short strict-pressure smoke passed with `fatal=0 warnings=1` at the intentionally low 700ms budget.

## 2026-04-29 - Restorable counter-win can beat passive open-four block

- Game: `local-ai-82c04954-c88c-4c2d-8ae4-6c5ac48c1ac9`, first confirmed teacher miss at computer move 38 after human `(7,4)#83`.
- Symptom: computer chose stable block `(5,4)#81` as `openFourDefense`. Mmai preferred `(9,9)#180`, which created a forced computer immediate win at `(9,8)#161`, removed the current fork pressure, and kept baseline non-winning capture pressure flat.
- Root cause: active open-four counter-wins reused the generic safe-four candidate list, which filtered out moves that did not improve active capture pressure. A broader candidate found `(9,9)`, but the reply verifier rejected it because one forced human response captures the new line and temporarily worsens capture pressure, even though the same move can be safely replayed and restores the immediate win.
- Fix or decision: under multiple active human open-four creation points, scan a bounded wider candidate slice for immediate counter-wins that keep capture pressure non-winning and reduce fork pressure. Allow a forced capture reply only when replaying the same counter move restores the immediate winning threat, leaves no human immediate or winning-capture reply, and returns open-four, fork, and capture pressure to no worse than the original baseline. Keep the broader path disabled for single open-four positions to preserve direct block regressions.
- Regression probe: compact fixture `counter-win may keep capture pressure flat when reducing fork pressure` expects `(9,9)#180` and rejects `(5,4)#81`; focused Mmai history `180,160,236,162,161,142,178,199,124,142,122,182,123,202,222,125,121,120,140,159,138,104,158,118,198,218,237,102,84,235,238,65,120,121,82,102,83` reports zero findings. Serial compact bench passed 22/22 in 74.18s; short strict-pressure smoke passed with `fatal=0 warnings=1` at the intentionally low 700ms budget.

## 2026-04-29 - Stretched four can force a capture-vulnerable block

- Game: `local-ai-a31d1c68-7839-4349-9116-05bb02660a3d`, computer move 16 after human `(10,12)#238`.
- Symptom: computer chose `(10,10)#200` as `compoundThreatPlacement`, a soft fork defense that ceded initiative. Move `(11,10)#201` was one cell to the right and created an immediate computer winning threat at `(10,10)#200`; the only human defense to that threat made the human pair capture-vulnerable.
- Root cause: `(11,10)` was outside the normal top candidate slice until line-threat candidates were added, and even after that it was rejected before scoring because the compound-threat gate counted only ordinary four/fork/capture classes. The forced capture-vulnerable response bonus was calculated after the `productiveClassCount >= 2` gate, so the stretched-four tactic never counted as its second productive class.
- Fix or decision: add a bounded own-line candidate expansion for capture-vulnerable forced fours, score moves that create an immediate computer win whose forced human defenses all increase computer capture pressure, and let that forced-response class satisfy the compound-threat productivity gate.
- Regression probe: compact fixture `stretched four forces capture-vulnerable block` expects `(11,10)#201` and rejects `(10,10)#200`; tactical probe selects `(11,10)` with reason `compoundThreatPlacement`. Serial compact bench passed 23/23 in 79.14s; short strict-pressure smoke passed with `fatal=0 warnings=2` at the intentionally low 700ms budget.

## 2026-04-29 - Safe row four can precede deferred capture defense

- Game: `local-ai-4ef4c21f-6cce-40a7-a54c-18608d789c24`, computer move 12 after human `(10,10)#200`.
- Symptom: computer chose `(11,11)#220` as `hardForcedDefense`, defending fork pressure while passing up `(7,8)#159`, which safely completed the row four and forced the human to answer first.
- Root cause: `safeFourBuildingMoves` already found `(7,8)`, but the normal safe-four reply checker rejected the line because the forced human block temporarily restored fork setup pressure before the computer's deferred capture defense. That made the AI spend the tempo on defense instead of taking a free forcing four.
- Fix or decision: let a safe four bypass the ordinary reply checker only when there is existing fork pressure, the move creates a computer immediate win, every forced human defense is non-losing, and the next computer move has a non-losing immediate capture follow-up. Keep the proof terminal for that shortcut so it stays under the advanced timeout. Guardrails added in the same pass: fork-capture defenses cannot leave more fork pressure than the best direct fork block, and active open-four counter-wins may leave open-four pressure only when they reduce live fork pressure.
- Regression probe: compact fixture `safe row four before deferred capture defense` expects `(7,8)#159` and rejects `(11,11)#220`; tactical probe selects `(7,8)` in about 3.04s. Serial compact bench passed 24/24 in 82.50s; short strict-pressure smoke passed with `fatal=0 warnings=2` at the intentionally low 700ms budget.

## 2026-07-10 - Search telemetry and pinned version comparison

- Game: measurement infrastructure; no single game id.
- Symptom: tactical regressions and teacher disagreements exposed individual mistakes, but there was no stable version-versus-version result or node/cache/depth evidence explaining why search behavior changed.
- Root cause: decision logs recorded phases and candidate scores without search node, completed-depth, transposition, cutoff, or stop-context telemetry; strength runs compared against moving behavior rather than an explicit code revision.
- Fix or decision: add DEBUG decision-log schema 4 `searchTelemetry`, a persistent release-mode move oracle, and `pente_ai_version_bench.py` with a full-commit baseline pin, fixed opening histories, paired side swaps, JSONL game records, and latency summaries. Keep low-budget runs as smoke tests and use the 4-second Advanced budget for promotion decisions.
- Regression probe: compile the oracle, send the tournament-center history, and run `python3 Scripts/pente_ai_version_bench.py --games 2 --max-moves 4 --time-limit-ms 100 --depth 2 --max-candidate-moves 8 --max-seconds 60`; then run the compact bench serially and verify a Debug simulator decision contains schema 4 `searchTelemetry`.

## 2026-07-10 - Empty fork-capture candidate lists must not build a fork baseline

- Game: `local-ai-7a6c407a-8700-40e5-9171-d06dc45d9981`, computer move 46 after human `(7,15)#292`.
- Symptom: Advanced selected valid compound defense `(6,15)#291`, but took 9,946ms. `forkResolvingCaptureDefenses` took 3,643ms and `forkResolvingImmediateCaptureThreatResponses` took 3,943ms even though both candidate groups were empty.
- Root cause: `forkResolvingCaptureDefenseMoves` constructed the same expensive direct-fork comparison baseline before checking whether it had any capture candidates, then repeated that work for the immediate-capture group.
- Fix or decision: deduplicate and reject empty candidate lists before baseline construction; share a bounded fork-resolution context and per-move result cache across both root phases; consult cached results before the deadline guard so an already-accepted defense is preserved. If the bounded direct-defense baseline cannot be completed, require complete fork-pressure resolution rather than relaxing the safety comparison.
- Regression probe: compact fixture `empty fork capture candidates avoid duplicate work` accepts the two stable open-four defenses `(6,15)#291` and `(10,15)#295`, asserts no immediate human win or open-four creation, and enforces a three-sample median CPU-time ceiling of 10,000ms so host scheduling contention does not turn wall-clock variance into a false compute regression. Under the same sustained host load, the exact pre-fix build measured 11,910ms median CPU versus 8,575ms after the fix; stable focused wall-clock runs measured 3,090-4,284ms. Final serial validation passed 25/25 in 100.33s with a 4,820ms median CPU result for the new fixture. The older `active blocker beats soft capture cleanup` fixture still took 12.51s CPU and remains the next deadline-profile target.
