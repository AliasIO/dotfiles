...
function runGh(options) {
  const fields = ghIssueFields(false).join(',')
  let command
  ...
  if (options.issue !== null) {
    const normalized = normalizeGhIssue(parsed)
    ...
    if (options.comments) {
      ...
      if (commentsResult.status !== 0) {
        fail(commentsResult.stderr.trim() || 'gh comment pagination failed')
      }

      const pages = JSON.parse(commentsResult.stdout)
      normalized.comments = pages.flat().map((comment) => ({
...
async function fetchJson(url, token) {
  const headers = {
    ...
    'User-Agent': 'codex-handle-wappalyzer-detection-issues',
  }
  ...
  if (!response.ok) {
    ...
    fail(`GitHub API request failed (${response.status}): ${body || response.statusText}`)
  }
...
async function fetchAllJson(initialUrl, token) {
  const items = []
  ...
  while (url) {
    if (visited.has(url)) {
      fail(`GitHub pagination loop detected at ${url}`)
    }
    ...
    if (!Array.isArray(page)) {
      fail('Expected a paginated GitHub API array response')
    }
...
async function runApi(options) {
  ...
  if (options.issue !== null) {
    ...
    if (options.comments && issue.comments > 0) {
      comments = await fetchAllJson(`${issue.comments_url}?per_page=100`, token)
    }
