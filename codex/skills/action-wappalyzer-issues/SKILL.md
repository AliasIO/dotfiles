...
5. Open the PR in the same GitHub repo that owns the issue you are actioning. The PR body must summarize what changed, list the example websites used for evidence, reference the original GitHub issue so the two are linked, and explain the fingerprint/metadata rationale (including meaningful issue-stub suggestions that were intentionally rejected or omitted and why).
...
7. If no code change is warranted, still close extension-intake tickets that are clearly `Not eligible`, `More info needed`, `Already added`, `Works as intended`, or `Won't fix` after applying the matching non-`Acknowledged` label. Use `More info needed` only after you have investigated the reporter's claim on the cited or comparable live pages and still hit a concrete blocker; do not shift the first pass of DOM/script/runtime evidence gathering onto the submitter. Before closing any ticket without `Accepted`, post a short issue comment that explains the reason and, for `More info needed`, the blocker you hit after investigating. Treat that comment as an internal triage note rather than a reply to the submitter: do not thank them, do not address them directly, and just state the reasoning for the label you applied. Every GitHub issue comment, PR comment, and PR body you post under the user's account must end with `— Codex`. Leave the ticket untouched only when it is truly outside the extension queue scope or belongs in another repo.
8. In your completion summary for the user, make every referenced issue and PR clickable with markdown links (for example `[ #16829 ](https://github.com/wappalyzer/wappalyzer/issues/16829)` and `[ #16834 ](https://github.com/wappalyzer/wappalyzer/pull/16834)`), not plain `#12345` text.
...
- Prefer `Fixed` only when you can point to an actual shipped code change (commit/PR on default branch) that resolved the reported issue.
- If no code change was made for the outcome, do not use `Fixed`; prefer `Works as intended` for non-reproducible/correct behavior, or `Won't fix` when the requested behavior is intentionally not supported.
...
- Leave a short issue comment when closing with any non-`Accepted` label, but write it as an internal triage note rather than a reply to the submitter: no thank-yous, no direct address, just the reason for the applied label.
