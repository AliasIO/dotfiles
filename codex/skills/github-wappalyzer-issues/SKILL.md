...
5. Before classifying a detection-failure report as blocked or later closing it as `More info needed`, investigate the reporter's claim on the cited or comparable live pages yourself. Do not shift the first pass of DOM/script/runtime evidence gathering onto the submitter.
...
14. If the user explicitly asks for GitHub-side actions such as opening a PR or posting issue comments, end each PR body and GitHub comment with a short `— Codex` signature because the text is being posted under the user's account.
