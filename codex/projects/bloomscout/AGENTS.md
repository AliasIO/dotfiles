...
- Require the `Website and backend` and `Android` CI checks before merging to `master`. Treat `master` as protected by policy even when the repository's GitHub plan cannot enforce branch protection. Keep unrelated local edits intact and never reset or clean them.
