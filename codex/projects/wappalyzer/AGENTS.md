...
- To avoid duplicate logical dataset writes during ping rollout, switch `PING_WRITE_MODE` from `shadow` to `async` at a UTC day boundary first, keep `PING_APPLY_MODE=compare` for that day, and only switch `PING_APPLY_MODE` to `apply` on the following UTC day because the compactor processes the previous UTC date.
