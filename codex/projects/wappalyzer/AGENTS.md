...
- Keep the `Customer APIs - API Gateway 4XX` widget on `Wappalyzer-Service-Groups` aligned with the existing `Customer APIs - API Gateway 5XX` widget's function set so both views cover the same APIs.
