...
- For Gmail add-on Apps Script auth, mirror the `Wappalyzer MCP Hosted` app client: use `https://mcp-auth.wappalyzer.com/oauth2/authorize` and `/oauth2/token`, request `openid email profile aws.cognito.signin.user.admin`, and use Apps Script OAuth2 PKCE without a client secret instead of the legacy `auth.wappalyzer.com/login` client.
