...
- In `cli/index.js`, run CloudFront block detection on any terminal document `4xx/5xx` response, not only the current request URL; redirected `403` deny pages can otherwise bypass the block short-circuit and exhaust the lookup Lambda budget.
...
- In `v4/apis/lookup/lookup.js`, when a live non-recursive analyze fails, run a short HTTP follow-up probe before returning the error so redirected deny pages can be normalized into handled block errors instead of generic browser-session failures.
