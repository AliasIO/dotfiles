...
- For suggestion tickets that appear niche, single-purpose, or low-user-value, prefer rejecting them at the eligibility gate before deep fingerprint research, even when sample sites or plausible fingerprints are provided.
...
- For Wappalyzer queue tickets that are in the normal extension-intake lane, investigate the reporter's claim on the cited or comparable live site before using `More info needed`; do not put the burden of the initial DOM/script/runtime evidence gathering on the submitter. If the ticket still cannot be actioned because it is not eligible or a concrete blocker remains after investigation, apply the matching terminal label such as `Not eligible` or `More info needed` and close the issue instead of leaving it open.
...
- When posting GitHub PR bodies, issue comments, or PR comments under the user's account, always end them with `— Codex` so the authored text is clearly attributed.
