...
For an explicit official documentation question, search and fetch the relevant official page. For local skills, configuration or code inspection, read the local source first and use official docs to resolve current facts or API contracts. Do not let source-order ceremony block independent offline work.
