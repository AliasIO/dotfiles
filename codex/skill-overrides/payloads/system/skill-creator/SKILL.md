...
The validator uses PyYAML when available, with Ruby/Psych as a fallback, and reports a clear dependency error if neither exists. It rejects empty required strings and unfinished scaffold placeholders. Default validation accepts supported host metadata and preserves existing title-case plugin names; use `--strict` for portable naming/frontmatter conventions when creating a new skill, and `--links` to check entrypoint-relative Markdown resources. Warnings about unknown host metadata need review against the actual host contract, not automatic deletion. Structural validation does not prove that the skill makes good decisions. Also check that descriptions remain discriminating, instructions preserve user intent, references are discoverable, and any added scripts actually work.
