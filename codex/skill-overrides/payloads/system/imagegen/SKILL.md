...
- Do not modify `scripts/image_gen.py` unless maintenance of that helper is requested. If a capability is missing, identify the affected action and use a permitted alternative or ask about that action; continue independent authorized work.
...
When constructing a detailed generation prompt, use [the schema and examples](references/prompt-schema-and-examples.md) as needed. Keep the user’s requested style and scope authoritative.
