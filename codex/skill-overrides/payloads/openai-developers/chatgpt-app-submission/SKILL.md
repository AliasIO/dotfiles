...
5. Compare tool annotations, tool names, tool descriptions, and CSP values against actual behavior. If any value is missing, stale, misleading, or inconsistent, update source when that correction is within the user’s already-authorized implementation scope; otherwise report the concrete proposed correction and ask only for the additional scope.
