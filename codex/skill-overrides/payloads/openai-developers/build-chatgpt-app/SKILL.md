...
## Documentation for the requested change

Inspect the existing local implementation and contract first. For a focused edit, fetch only the current official documentation relevant to the changed API behavior; the baseline list below is for new scaffolds or broad contract changes. Offline work can continue while credentials are unresolved.

Use `$openai-docs` for current Apps SDK guidance, following its distinction between local inspection and official documentation questions.

1. Invoke `$openai-docs` (preferred) or call the OpenAI docs MCP server directly.
2. Fetch current Apps SDK docs for the changed contract before relying on an API shape. For a new app, use these baseline pages:
