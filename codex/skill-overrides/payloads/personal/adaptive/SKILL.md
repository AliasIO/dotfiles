...
Inspect whether the affected screen uses Compose, Views or a hybrid stack, and which navigation library it already uses. Adapt the requested layout within that stack. The Compose/Navigation 3 procedures below apply when those APIs are already available or their adoption is explicitly part of the task. Do not make a navigation or UI-framework migration a prerequisite for a small adaptive fix. For Views or other navigation stacks, use their current official adaptive-layout guidance.
...
For projects already using Navigation 3, prefer its `SceneStrategy` approach for
multi-pane layouts. For other existing stacks, retain a compatible scaffold such as `ListDetailPaneScaffold` or
...
Honor any existing decision on experimental APIs. If adoption would be new and material, explain the tradeoff and ask; otherwise use a compatible stable approach.
