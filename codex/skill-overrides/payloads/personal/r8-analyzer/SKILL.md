...
- **Report**: Derive quantitative scores and impact metrics from `analysis.txt`. Record the actual tool version. If generation requires out-of-scope build changes or an unavailable capability, use the clearly labeled heuristic path instead of modifying project configuration during an audit.
...
- **Output** : Output ONLY the raw Markdown report in the chat. Do NOT output conversational filler (for example, "Here is your report..."). Do NOT provide recommendations, next steps, or any other text outside of the sections defined in [references/REPORT_FORMAT.md](references/REPORT_FORMAT.md) State whether findings use generated quantitative evidence or heuristic inspection, including any capability limitation
