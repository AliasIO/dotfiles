...
Resolve an existing `trace_processor` from PATH or a known task-local cache. If unavailable, download the official wrapper from `https://get.perfetto.dev/trace_processor` into task scratch storage, inspect it, and use its absolute path as `$TRACE_PROCESSOR`. Do not add analysis tools to the repository root or edit `.gitignore` just to analyze a supplied trace. Avoid broad filesystem searches; check known locations. Record the actual processor version and inspect the trace schema before relying on version-dependent modules. The official wrapper is Python and downloads its binary on first use.
