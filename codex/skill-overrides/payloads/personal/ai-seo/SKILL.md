---
name: ai-seo
description: "Assess and improve a website’s discoverability and citations in AI search. Use for AI visibility, answer-engine optimization or agent-readable content; use seo-audit for ordinary technical SEO."
metadata:
  version: 2.1.0
---
...
AI-search behavior and citation frequency vary by engine, query set, market, and date. Do not promise a visibility or traffic lift from a tactic. See [evidence and emerging formats](references/evidence-and-emerging-formats.md) before using study numbers or recommending a new machine-readable format.

Write for people, provide original evidence, make source attribution clear, and verify current platform guidance for the target engine. Structure should improve comprehension; a fixed passage length or extra file is not a universal ranking requirement.
...
- Keep key answer passages concise enough to answer the question without losing essential context
...
The 2024 GEO research is historical experimental evidence, not a current cross-platform performance guarantee. Read [the source note](references/evidence-and-emerging-formats.md) when discussing its findings.
...
**Relevant third-party sources to investigate for the target audience:**
- Accurate, independently supported Wikipedia coverage when relevant
...
Use schema that accurately describes visible content and verify current platform eligibility. Valid markup does not guarantee AI citations or rich results. For implementation, use the **schema** skill.
...
Choose content formats from the user's question and available evidence: comparisons for choices, original research for new findings, product pages for current specifications, and guides for explanations or procedures. Treat citation-share estimates as dataset-specific observations, not fixed publishing targets.
...
- **Keyword stuffing** — prioritize useful, natural writing; do not claim a fixed visibility penalty across engines
...
Use available connectors according to [shared marketing context](../_shared/marketing-context.md); do not assume a local registry is installed.
