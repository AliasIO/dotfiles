...
**Resolve the processor:** Reuse a known executable from the project or PATH, checking only expected locations. Otherwise download the official wrapper into task-local scratch/cache storage. Record its absolute path and version; do not add a tool to the repository or change `.gitignore` for an analysis task.
...
Check the selected executable directly. If missing, save `https://get.perfetto.dev/trace_processor` to the chosen scratch/cache path, make it executable where needed, and invoke it by absolute path (on Windows, through the appropriate Python runtime). Use current processor help and available schema to resolve documentation/version differences.
...
- \[ \] **Execution Check:** You MUST run queries using the standalone
  resolved `trace_processor` wrapper with the `--query-string` flag:
  `"$TRACE_PROCESSOR" --query-string "QUERY" "<trace-file>"`, where `TRACE_PROCESSOR` is the resolved absolute executable path.

  **Execution Rules:**
  - **File Usage** : If you must create a SQL file to execute queries (for example, due to query length or escaping issues), create them in task-local temporary storage.
...
2. Before finishing your response, remove temporary SQL files that are no longer needed, retaining a requested reproducible query artifact.
