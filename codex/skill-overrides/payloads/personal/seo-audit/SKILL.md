...
Raw HTTP HTML can provide positive evidence of server-rendered JSON-LD. Text-extracted fetch output may omit script elements; JavaScript-generated markup may require rendered DOM inspection. Record which representation was checked. If the available representation does not establish presence or absence, inspect `script[type="application/ld+json"]` in the rendered page or use the current Rich Results Test. Do not conclude that schema is absent from incomplete evidence.
