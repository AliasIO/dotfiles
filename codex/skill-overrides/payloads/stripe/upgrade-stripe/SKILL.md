...
Resolve the user’s requested Stripe API/SDK target from current official documentation. Preserve an explicit target; for an unspecified upgrade, check supported versions and compatibility before editing. Version literals in examples below are dated illustrations, not a claim about the latest release.
