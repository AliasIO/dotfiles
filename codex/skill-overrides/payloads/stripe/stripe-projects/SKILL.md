...
description: "Provision services through Stripe Projects when that route is requested or explicitly selected after inspecting existing provider/project constraints. Do not treat a generic database, hosting or API-key request as authority to choose a new provider."
allowed-tools:
  - Bash(stripe *)
  - Bash(which stripe)
...
First inspect existing providers, infrastructure and credential conventions. Use Stripe Projects only when requested or selected within the user’s scope. Preserve named providers. Explain and resolve a material provider/account choice before provisioning; do not install CLI components merely because the request mentions a database or API key.
