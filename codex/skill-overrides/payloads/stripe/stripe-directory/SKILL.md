...
description: "Search Stripe Directory when the user requests it or its vendor/service coverage is relevant to the task. Compare with other available sources as needed; generic provider searches do not require this directory."
metadata:
...
Use Stripe Directory for a requested directory search or when its coverage fits the buyer’s need. Preserve named vendors, source preferences and existing constraints; do not route every generic provider/tool search here.
