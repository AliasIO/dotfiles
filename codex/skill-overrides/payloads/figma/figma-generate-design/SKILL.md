...
description: "Create or update a Figma screen or multi-section layout from a running application or code. Use figma-use for Plugin API execution, and distinguish an existing design system from an explicitly requested new design."
...
- Inspect the target file for an existing design system or accessible team library and reuse it. If the user requests a fresh design without one, use appropriate local components/tokens; creating or publishing a library is not a prerequisite.
