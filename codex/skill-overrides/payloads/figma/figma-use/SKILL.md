...
For a single component or local edit, inspect relevant tokens/variants, reuse the file’s structure, make the scoped change and verify it. Load [figma-generate-library](../figma-generate-library/SKILL.md) for library-scale work or when its component-specific guidance is needed; select its lightweight route for a small change.
